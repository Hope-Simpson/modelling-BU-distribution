...
path_outputs <- "C:/Users/Hope/Dropbox (LSoHaTM)/BU_Modelling/Outputs/BU_clinical_cases"
...
setwd(path_wd)
AFRO_BU <- file.path(path_shapefiles, "/BU_occurrences/BU_occurrences_all.shp")
...
## 2.4 Load the background points   created in 5_KDE_surface_all.R
BG <- file.path(path_BG, "KDE_BG_all.shp")    
BG <- st_read(BG)
...
cov.5km <- readRDS("covariates_5km_alloc_v4.rds")
...
getwd()
set.seed(123)
system.time(BU.model.all <- BIOMOD_Modeling(BU.Formatted,
...
BU.model.all # Model failing: none

## 7.1. Extract evaluations from all models run
BU.model.all.Eval <- get_evaluations(BU.model.all)
dimnames(BU.model.all.Eval)
...
performance.models1 <- models_scores_graph(BU.model.all, 
                                           by = "models", 
                                           metrics = c("ROC","TSS"))
...
performance.models2 <- models_scores_graph(BU.model.all, 
                                           by = "models", 
...
TSS.test.data <- as.data.frame(t(BU.model.all.Eval["TSS","Testing.data",,,]))
ROC.test.data <- as.data.frame(t(BU.model.all.Eval["ROC","Testing.data",,,]))
PCC.test.data <- as.data.frame(t(BU.model.all.Eval["ACCURACY","Testing.data",,,]))
...
BU.model.all.Var <- get_variables_importance(BU.model.all)
dimnames(BU.model.all.Var)

Variable.Contribution <- t(apply(BU.model.all.Var, 2, function (x) rowMeans(x, na.rm = TRUE)))

write.csv(Variable.Contribution, file = "Variable_Contribution_BU_all.csv")
...
BU.RF <- BIOMOD_LoadModels(BU.model.all, models = "RF")

RFsPlot2D <- response.plot2(models = BU.RF, 
                            Data = get_formal_data(BU.model.all,"expl.var"), 
                            show.variables = get_formal_data(BU.model.all, "expl.var.names"), 
                            do.bivariate = FALSE, 
                            fixed.var.metric = "median", 
                            col = c("blue", "red"),
                            legend = TRUE, 
                            data_species = get_formal_data(BU.model.all, "resp.var"))

rm(list = ls(pattern = "BU_AllData")) 


select_models <-  BU.model.all@models.computed
# 8. Ensemble Modelling. Come up with a consensus model and ensemble for every type of model
BU.model.all.Ensemble <- BIOMOD_EnsembleModeling(modeling.output = BU.model.all,
...
BU.Ensemble.Eval <- get_evaluations(BU.model.all.Ensemble)
...
BU.Proj <- BIOMOD_Projection(modeling.output = BU.model.all,
                              new.env = covariates,
                              proj.name = "Final",
...
BU.EF <- BIOMOD_EnsembleForecasting(EM.output = BU.model.all.Ensemble,
...
setwd(path_outputs)
writeRaster(BU.error, filename = "BU.error_all.tif", format= "GTiff", overwrite=TRUE)


# 11. Explore correlation between  Model2 PCR and all cases
### Load the predicted environmental suitability for BU, accoridng to model1
setwd(path_outputs)
BU.WM.all  <- raster("BU_WM.tif")

setwd("C:/Users/Hope/Dropbox (LSoHaTM)/BU_Modelling/Outputs/BU_confirmed_cases")
BU.WM.PCR <- raster("BU_WM.tif")

PCS <- BU.WM.all@crs

BU.WM.PCR <- projectRaster(BU.WM.PCR, crs = PCS)
BU.WM.PCR <- extend(BU.WM.PCR, BU.WM.all)
BU.WM.PCR <- crop(BU.WM.PCR, BU.WM.all)
BU.WM.PCR <- resample(BU.WM.PCR, BU.WM.all)

BU.WM <- stack(BU.WM.PCR, BU.WM.all)
names(BU.WM) <- c("BU.WM.PCR","BU.WM.all")

# Correlation between layers

Cor.BU.Models <- layerStats(BU.WM, 'pearson', na.rm=T)
Cor.BU.Models$'pearson correlation coefficient' ## Corr. Coef:  .86
